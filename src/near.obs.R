near.obs <-
  function (locations,
    ...
    if (class(locations) == "SpatialPoints" || class(locations) ==
    ...
    if (class(observations) == "SpatialPoints" ||
        class(observations) ==
        "SpatialPointsDataFrame") {
    ...
    if (nrow(observations) < (n.obs + 1)) {
      nl_df <- matrix(NA, nrow = nrow(locations), ncol = (2 *
                                                            n.obs))
    }
      ...
      if (rm.dupl) {
        knn1 <- nabor::knn(observations, locations, k = n.obs +
                             1)
        knn1$nn.idx[round(knn1$nn.dists[, 1]) == 0, 1:n.obs] <-
          knn1$nn.idx[round(knn1$nn.dists[,
                                          1]) == 0, -1]
        knn1$nn.idx <- knn1$nn.idx[, -(n.obs + 1)]
        knn1$nn.dists[round(knn1$nn.dists[, 1]) == 0, 1:n.obs] <-
          knn1$nn.dists[round(knn1$nn.dists[,
                                            1]) == 0, -1]
      ...
      if (any(class(knn1$nn.idx) != "integer")) {
        near_o1 <- apply(knn1$nn.idx, 2, function(x) {
          variable[x]
        })
      ...
      for (ip in idw.p) {
        if (nrow(observations) < (n.obs + 1)) {
          ...
          if (any(class(knn1$nn.dists) == "numeric")) {
            ...
            idw.w <-
              t(apply(knn1$nn.dists, 1, function(x)
                (1 / (x) ^ ip) / sum(1 / (x) ^ ip)))
            nl_df <- cbind(nl_df, apply(idw.w * near_o1,
                                        1, sum))
          }
        }
    ...
    if (avg) {
      knn2 <- nabor::knn(observations,
                         locations,
                         k = nrow(observations),
                         radius = range)
      if (rm.dupl) {
        knn2$nn.idx[round(knn2$nn.dists[, 1]) == 0, 1:(nrow(observations) -
                                                         1)] <-
          knn2$nn.idx[round(knn2$nn.dists[, 1]) ==
                        0, -1]
        knn2$nn.idx <- knn2$nn.idx[, -(nrow(observations))]
        knn2$nn.dists[round(knn2$nn.dists[, 1]) == 0, 1:(nrow(observations) -
                                                           1)] <-
          knn2$nn.dists[round(knn2$nn.dists[, 1]) ==
                          0, -1]
        knn2$nn.dists <- knn2$nn.dists[, -(nrow(observations))]
      }
      else {
        knn1 <- nabor::knn(observations,
                           locations,
                           k = nrow(observations),
                           radius = range)
      }
      knn2$nn.idx <- ifelse(knn2$nn.idx == 0, NA, knn2$nn.idx)
      near_o2 <- apply(knn2$nn.idx, 2, function(x) {
        variable[x]
      })
      ...
      for (inc in 1:nc) {
        avg_st[, inc] <- apply(ifelse((knn2$nn.dists <= inc *
                                         increment) &
                                        (knn2$nn.dists >= (inc - 1) * increment),
                                      near_o2,
                                      NA
        ), 1, "mean", na.rm = T)
      }
      avg_st[, nc] <- ifelse(is.na(avg_st[, nc]), mean(variable),
                             avg_st[, nc])
      for (inc in (nc - 1):1) {
        avg_st[, inc] <- ifelse(is.na(avg_st[, inc]), avg_st[,
                                                             (inc + 1)], avg_st[, inc])
      }
      nl_df <- cbind(nl_df, avg_st)
      name3 <- paste("avg", trimws(format(
        seq(increment, range +
              increment - 1, increment), scientific = F
      )), sep = "")
    ...
    if (direct) {
      ...
      direct_obs[, 1] <-
        sapply(1:nrow(locations), function(x)
          near_o2[x,
                  (observations[knn2$nn.idx[x,], "x"] > locations[x,
                                                                  "x"] &
                     observations[knn2$nn.idx[x,], "y"] >
                     locations[x, "y"])][1])
      direct_obs[, 2] <-
        sapply(1:nrow(locations), function(x)
          near_o2[x,
